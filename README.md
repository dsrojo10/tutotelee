...
Sin `?debug=1` no se recopila ni se muestra el panel ni se añade el observador de presencia. Los mensajes de producción siguen siendo genéricos. El diagnóstico no añade escrituras ni modifica reglas o arquitectura.
...
### Reloj y expiración: hallazgo del Samsung

El Samsung reportó autenticación y conexión RTDB correctas, `get(.info/serverTimeOffset)` con `Invalid token in path` y `set(sessionRef)` con `PERMISSION_DENIED`. El cálculo anterior sumaba el TTL máximo al reloj local si fallaba la lectura del offset. Si el adelanto local supera el tiempo transcurrido hasta validar la escritura, ese cálculo excede `now + TTL` y las reglas lo rechazan. Las pruebas del emulador reproducen el rechazo con 30 segundos de adelanto y verifican la aceptación con el margen, para sesión y pairing. Esto confirma un defecto real, pero no mide el desfase concreto del Samsung.

`getServerNow()` ahora usa `ref(database, '.info/serverTimeOffset')` y `onValue()` modular. Espera el primer valor numérico finito (cero también es válido), ignora valores inválidos y desuscribe al resolver, cancelar o vencer el timeout de 5 segundos. También limpia cuando Firebase entrega el valor síncronamente desde caché. Devuelve `{ now, source }`; en caso de error o timeout, `source` es `local-fallback`, conserva el error en debug y usa la hora local tomada al resolver.

Con `server-offset`, las expiraciones usan el TTL normal. Con `local-fallback`, `calculateExpiresAt()` resta `CLOCK_SKEW_SAFETY_MS = 60000`: un minuto de tolerancia a un reloj adelantado, tanto al crear la sesión y pairing como al activar la sesión desde el celular. Así, con reloj correcto y fallback, el pairing dura 9 minutos y la sesión 11 horas 59 minutos. No cambian los límites de reglas (10 minutos y 12 horas). Este margen no cubre relojes arbitrariamente incorrectos; un adelanto mayor de un minuto todavía puede causar rechazo y un reloj muy atrasado puede producir expiración pasada.

La inspección de `@firebase/database` 1.1.5 instalado (`dist/index.esm.js`) muestra que `repoGetValue()` usa `serverSyncTree_` y, sin caché, `repo.server_.get(query)`. En cambio, `repoAddEventCallbackForQuery()` dirige las rutas `.info` a `infoSyncTree_`, alimentado con metadatos de conexión; `handleTimestamp_()` calcula el offset recibido del servidor. Por eso una lectura remota de `.info` es una explicación compatible con `Invalid token in path`, mientras el listener de `.info/connected` funciona. El código no prueba que sea un defecto exclusivo de Tizen ni permite confirmar el origen exacto de la respuesta del servicio sin reproducirlo allí.

Firebase documenta precisamente [onValue para estimar el desfase del reloj](https://firebase.google.com/docs/database/web/offline-capabilities#clock_skew) y advierte que el offset es una estimación afectada por latencia. El cambio evita la ruta de `get()` para metadatos sin modificar internamente el SDK. Quedan por comprobar en el Samsung el nuevo offset y la aceptación de la escritura; si falla aun con hora válida, revisar desfases mayores, precisión del offset, reglas efectivamente desplegadas y otras validaciones de la escritura. No se han cambiado target, SDK, Authentication, App Check, CSP, esquema ni protocolo de emparejamiento.

## Experimento: ventana rodante de voz

La rama `feature/voice-rolling-window` parte del punto estable `7e5ccea59864e277189e8332551d3c93fabdbb5a` de `main`. El experimento no se despliega a GitHub Pages al hacer push de la rama: el workflow publica únicamente pushes a `main` o ejecuciones manuales. Para probarlo localmente, cambia a esta rama y usa `npm run dev` (o `npm run build` y `npm run preview`). Para probar voz en un celular real, usa un origen HTTPS permitido por Firebase y accesible desde el dispositivo; no se añaden dominios ni servicios de despliegue en este cambio.

Solo la voz usa `VOICE_WINDOW_MAX_CHARS = 420`, dejando 80 caracteres de holgura frente al máximo Firebase. Se conservan hasta cuatro resultados finales recientes más el interim actual, unidos por un salto de línea. Si no caben, se retiran finales completos desde el más antiguo; también se retira el más antiguo al recibir un quinto final. Es una memoria breve, no un historial. El interim siempre tiene prioridad y puede desplazar todo el contexto. Un segmento gigante conserva su cola: se evita empezar con media palabra si hay un espacio disponible; una palabra que excede todo el límite se recorta por necesidad. El buffer de la aplicación permanece acotado, aunque el navegador entregue un array acumulativo de resultados.

Los finales de un mismo ciclo de reconocimiento se incorporan una sola vez; el interim se reemplaza o retira según los eventos del navegador. Las pausas no borran contexto ni requieren timers adicionales. `userWantsListening` mantiene DETENER disponible durante los reinicios automáticos. Después de `end`, se espera 750 ms antes del primer reinicio, 1500 ms antes del segundo y 2250 ms antes del tercero si no aparecen resultados. Tras tres reinicios sin resultados se muestra un mensaje y hace falta pulsar HABLAR. Un resultado no vacío restablece este contador. Solo `no-speech`, `network` y `aborted` son recuperables; los permisos denegados y otros errores detienen el intento. Nunca se inicia otra captura antes de `end`.

DETENER, BORRAR TV, desconectar, terminar la sesión o salir de la página cancelan el timer y limpian el contexto y el interim internos. BORRAR también detiene el micrófono, evitando que un resultado tardío restaure texto borrado. DETENER conserva lo último mostrado en el TV, pero la siguiente activación manual empieza con contexto vacío. Una pausa/reanudación automática conserva los finales, sin convertir un interim inconcluso en final. El texto manual y sus saltos de línea siguen usando exactamente la normalización y el límite de 500 anteriores. No cambian Firebase, pairing, reloj/TTL, reglas ni presentación del TV.

### Prueba de experiencia antes de aprobar

1. Abre TV, crea el código y conecta el celular desde el mismo entorno experimental.
2. Envía texto manual con líneas vacías, comprueba su formato y BORRAR TV.
3. Pulsa HABLAR y habla más de 500 caracteres: lo último debe seguir apareciendo, desplazando ideas antiguas.
4. Haz pausas y prueba una frase muy larga sin pausa. La vista previa y el TV deben conservar la parte reciente.
5. Realiza varios ciclos rápidos de DETENER/HABLAR, y prueba BORRAR, desconexión y reconexión. No debe reaparecer contexto descartado ni reiniciarse el micrófono después de detenerlo.
6. Deniega permisos de micrófono: no debe haber reintentos automáticos.

Las pruebas automatizadas usan resultados de voz y timers simulados, sin tiempos reales ni servicios de reconocimiento externos. La calidad de reconocimiento y si el navegador permite reiniciar automáticamente tras una pausa requieren esta prueba en el celular real. No se ha hecho merge a `main`.
